...
class Post extends BaseController
{
    ...
    public function index()
    {
        ...
        $search = $this->request->getGet('search'); // ค้นหา
        $category = $this->request->getGet('category'); // หมวดหมู่
        $tag = $this->request->getGet('tag'); // แท็ก
        $sort = $this->request->getGet('sort') ?? 'latest'; // จัดเรียง (default: latest)
        $page = $this->request->getGet('page') ?? 1; // หน้า (default: 1)
        ...
        // ดึงโพสต์ล่าสุด
        $recentPosts = $this->postModel->getRecentPosts(); // เรียกฟังก์ชัน getRecentPosts()

        $data = [
            'title' => 'โพสต์ทั้งหมด - ShareHub',
            'posts' => $posts,
            'recentPosts' => $recentPosts, // ส่งผลลัพธ์ของโพสต์ล่าสุดไปยัง View
            'categories' => $this->categoryModel->getActiveCategories(), // ดึงหมวดหมู่ที่เปิดใช้งานพร้อมจำนวนโพสต์
            'trending_tags' => $this->tagModel->getPopularTags(), // ดึงแท็กยอดนิยม
            'pager' => $this->postModel->pager, // ระบบแบ่งหน้า
            'current_category' => $category, // หมวดหมู่ที่เลือกปัจจุบัน
            'current_tag' => $tag, // แท็กที่เลือกปัจจุบัน
            'current_search' => $search, // คำค้นหาปัจจุบัน
            'current_sort' => $sort, // ตัวเลือกการจัดเรียงปัจจุบัน
        ];
    ...
    public function delete($id)
{
    ...
    if (!$this->canEditPost($post)) {
        // หากไม่มีสิทธิ์ลบโพสต์ ให้ redirect กลับไปที่หน้า /post พร้อมกับแจ้งเตือน
        return redirect()->to('/post')->with('error', 'คุณไม่มีสิทธิ์ลบโพสต์นี้');
    }

    if ($this->postModel->delete($id)) {
        // หากลบสำเร็จ ให้ redirect กลับไปที่หน้า /post พร้อมกับแจ้งเตือน
        return redirect()->to('/post')->with('message', 'ลบโพสต์เรียบร้อยแล้ว');
    }

    // หากเกิดข้อผิดพลาดในการลบโพสต์
    return redirect()->to('/post')->with('error', 'เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง');
}
    ...
    {
        // ตรวจสอบว่าเป็นเจ้าของความคิดเห็นหรือแอดมิน
        $comment = $this->commentModel->find($commentId);
        
        // ตรวจสอบว่าเจอความคิดเห็น และตรวจสอบสิทธิ์
        if ($comment && (session()->get('user_id') == $comment['user_id'] || session()->get('user_role') == 'admin')) {
            
            // ลบความคิดเห็น
            $this->commentModel->delete($commentId);
    
            // ส่งข้อความสำเร็จไปยังผู้ใช้
            // หากมี Referer ให้ redirect กลับไปที่หน้าที่ผู้ใช้มาจาก
            $referer = $this->request->getServer('HTTP_REFERER');
            if ($referer) {
                return redirect()->to($referer)->with('message', 'ความคิดเห็นถูกลบแล้ว');
            } else {
                // หากไม่มี Referer ก็ให้กลับไปที่หน้าโพสต์เดิม
                return redirect()->to(site_url('post/view/' . $comment['post_id']))
                                 ->with('message', 'ความคิดเห็นถูกลบแล้ว');
            }
        }
    
        // หากไม่มีสิทธิ์ ลองส่งกลับไปยังหน้าหลัก
        return redirect()->to(site_url())->with('error', 'คุณไม่มีสิทธิ์ในการลบความคิดเห็นนี้');
    }
