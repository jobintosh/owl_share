...
public function updateComment($commentId)
{
    try {
        ...
        $isAdmin = $user['role'] === 'admin'; // ตรวจสอบ role จากฐานข้อมูล

        // ดึงข้อมูลความคิดเห็น
        $comment = $this->commentModel->find($commentId);
        if (!$comment) {
            throw new \RuntimeException('ไม่พบความคิดเห็น');
        }

        // ตรวจสอบว่าเป็นเจ้าของหรือ Admin
        if ($comment['author_id'] != $userId && !$isAdmin) {
            throw new \RuntimeException('คุณไม่มีสิทธิ์แก้ไขความคิดเห็นนี้');
        }
...
{
    try {
        // Verify CSRF Token using CodeIgniter's Security Service
        $request = \Config\Services::request();
        $security = \Config\Services::security();

        // Retrieve CSRF token from headers or POST data
        $tokenName = $security->getTokenName();
        $token = $request->getHeaderLine('X-CSRF-TOKEN') ?: $request->getPost($tokenName);

        // Validate the token (FIXED ARGUMENTS)
        if (!$token || !$security->verify($request, $token)) { // Pass $request first
            throw new \RuntimeException('Invalid CSRF Token');
        }
        ...
        if ($comment['author_id'] != $userId && !$isAdmin) {
            throw new \RuntimeException('คุณไม่มีสิทธิ์ลบความคิดเห็นนี้');
        }
