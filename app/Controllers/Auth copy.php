...
use CodeIgniter\I18n\Time; //เพิ่ม
use App\Models\UserModel; // เพิ่มบรรทัดนี้
...
class Auth extends BaseController
{
    ...
    public function doLogin()
    {
        ...
        // //  อัพเดทในฐานข้อมูล
        // $sql = "UPDATE user SET password = '$hash' WHERE email = 'shop1@example.com';";
        // echo $sql;
        // exit;
        ...
        if ($user['status'] !== 'active') {
            return redirect()->back()->withInput()->with('error', 'บัญชีของคุณถูกระงับ กรุณาติดต่อผู้ดูแลระบบ');
        }
        ...
        // fix not re route to profile page || do this when uncheck remember me || if check will stuck auth/dologin page
        return redirect()->to('/')->with('success', 'เข้าสู่ระบบสำเร็จ');
    }
    ...
    public function doregister()
    {
        ...
        $userData = [
            ...
            'email' => $this->request->getPost('email'),
            'password' =>  $this->request->getPost('password'),  //password_hash($this->request->getPost('password'), PASSWORD_DEFAULT),
            'role' => 'user',
            'status' => 'active'
        ];
    ...
    // Protected Methods
    protected function handleSocialLogin($provider, $socialUser)
    {
        ...
        // ตรวจสอบว่ามีการเชื่อมต่อ Social Login นี้แล้วหรือไม่
