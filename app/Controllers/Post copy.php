...
class Post extends BaseController
{
    ...
    public function index()
    {
        ...
        // อัพเดทคอมเม้นต์แต่ละโพสต์
        $postModel = new PostModel();

        // ดึงข้อมูลโพสต์ทั้งหมด
        $posts = $postModel->findAll();

        // อัปเดต comment_count สำหรับแต่ละโพสต์
        foreach ($posts as $post) {
            $postModel->updateCommentCount($post['id']);
        }
    ...
    //     // ถ้าไม่พบโพสต์หรือโพสต์ถูกลบ
    //     if (!$post) {
    //       //  throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound('ไม่พบบทความที่คุณต้องการ');
    //     }

    //     // ตรวจสอบสถานะโพสต์
    //     if ($post['status'] !== 'published' && 
    //         (!session()->get('user_id') || 
    //         (session()->get('user_id') !== $post['author_id'] && 
    //          session()->get('role') !== 'admin'))) {
    //         throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound('บทความนี้ยังไม่ได้เผยแพร่');
    //     }
    ...
    //         $post['id'],
    //         $post['category_id'],
    //         6 // จำนวนโพสต์ที่เกี่ยวข้อง
    //     );

    //     // ดึงความคิดเห็นพร้อมการตอบกลับ
    ...
    //         } catch (\Exception $e) {
    //             $post['tags'] = [];
    //             log_message('error', 'Error decoding post tags: ' . $e->getMessage());
    //         }
    //     } else {
    //         $post['tags'] = [];
    //     }
